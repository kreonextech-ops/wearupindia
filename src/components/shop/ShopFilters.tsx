    ...
    <div className="flex flex-col gap-10">
      ...
      <div className="relative group">
        ...
        <input
          ...
          className="w-full bg-muted/30 border border-border text-foreground font-mono text-[11px] placeholder:text-muted-foreground/30 pl-11 pr-4 py-4 focus:outline-none focus:border-wu-red group-focus-within:bg-muted/50 transition-all tracking-widest"
      ...
      <div>
        <label className="block font-mono text-[10px] text-wu-red tracking-[0.25em] uppercase mb-4 font-bold flex items-center gap-2">
           <span className="w-4 h-px bg-wu-red" /> Collection
        </label>
        <div className="flex flex-col gap-3 max-h-[250px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-border">
          {brandOptions.length > 0 ? brandOptions.map((brand) => (
            <button
              ...
              className={`flex items-center justify-between group py-1 text-left transition-all ${
                filters.brands.includes(brand) ? 'text-foreground font-bold' : 'text-muted-foreground hover:text-foreground'
              }`}
            >
              ...
              <div className={`w-3.5 h-3.5 border transition-all flex items-center justify-center ${
                filters.brands.includes(brand) ? 'bg-wu-red border-wu-red' : 'border-border'
              }`}>
                {filters.brands.includes(brand) && <div className="w-1.5 h-1.5 bg-white" />}
              </div>
            </button>
          )) : (
            <p className="text-muted-foreground/40 font-mono text-[10px]">NO OPTIONS AVAILABLE</p>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="px-1">
          <input
            ...
            max={maxPrice}
            step="100"
            className="w-full h-1.5 bg-muted/60 rounded-full appearance-none cursor-pointer accent-wu-red"
            value={filters.priceRange[1]}
            onChange={(e) => handleChange({ ...filters, priceRange: [0, parseInt(e.target.value)] })}
          />
          <div className="flex items-center justify-between mt-4">
            <span className="font-mono text-[10px] text-muted-foreground tracking-widest uppercase">Max: ₹{filters.priceRange[1].toLocaleString()}</span>
            <button 
              onClick={() => handleChange({ ...filters, priceRange: [0, maxPrice] })}
              className="font-mono text-[9px] text-wu-red hover:underline"
            >
              RESET
            </button>
          </div>
